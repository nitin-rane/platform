...
create table if not exists landing.ga_hits_homegate_apps_history(
...
cluster by (dt)
comment='This table stores immutable data that is extracted from a Google Big query source in JSON format. The data is for homegate apps. Google dataset ID number is 106414467'
;     
